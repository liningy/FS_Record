...
public:
	...
	BYTE*			Kinect_getAlphaPixels(); // store byte pointers pointing to R,G,B,A for each pixel; in total it has 4 * width * height
	BYTE*			Kinect_getRGBBuffer();   // store byte pointers pointing to R,G,B for each pixel;   in total it has 3 * width * height
	USHORT*			Kinect_getDepthBuffer(); // store unsigned short (2 bytes) pointers pointing to each pixels's depth in milimeter; in total it has 1 * width * height
